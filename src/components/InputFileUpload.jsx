...
export default function InputFileUpload( {userImage, setUserImage, userPrompt, setUserPrompt} ) {  
      ...
      onChange={(event) => {
        if (event.target.files && event.target.files[0]) {
          if (event.target.files[0].size > 1 * 1000 * 1024) {
            ...
            let userPromptCopy = userPrompt.slice();
            userPromptCopy[0] = "Only single-file upload is supported at this time"
            userPromptCopy[1] = -1;
            setUserPrompt(userPromptCopy);
          }
          else {
